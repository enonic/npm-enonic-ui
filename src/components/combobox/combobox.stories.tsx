...
const PopupApplyBar = (): ReactElement | null => {
  ...
  return (
    <div className='flex justify-end border-bdr-subtle border-t p-2'>
      <Combobox.Apply />
    </div>
  );
};
...
export const StagedApplyInPopup: Story = {
  ...
  render: () => {
    ...
    return (
      <div className='relative w-80 space-y-3'>
        <header>
          <h3 className='font-medium text-md'>Apply lives inside the popup</h3>
          <p className='text-sm text-subtle'>
            Stage a change, then <kbd className='rounded bg-surface-primary px-1 font-mono text-xs'>Tab</kbd> to Apply
            and <kbd className='rounded bg-surface-primary px-1 font-mono text-xs'>Tab</kbd> again — focus must escape.
          </p>
        </header>
        <Combobox.Root value={value} onChange={setValue} selectionMode={'staged'}>
          <Combobox.Content>
